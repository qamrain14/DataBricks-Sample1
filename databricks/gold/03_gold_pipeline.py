...
from pyspark.sql.functions import (
    col, when, trim, upper, lit, current_timestamp, current_date, to_date,
...
def gold_dim_vendor():
    df = spark.readStream.table(f"{CATALOG}.procurement_silver.silver_vendors")
    return df.select(
        col("vendor_id"),
        col("vendor_code"),
        col("vendor_name"),
        ...
        col("prequalification_expiry_date").alias("prequalification_expiry"),
        col("iso_certified"),
        col("hse_rating"),
        col("minority_owned"),
        col("small_business"),
        col("contact_name").alias("primary_contact"),
        col("contact_email"),
        col("contact_phone"),
...
def gold_dim_project():
    df = spark.readStream.table(f"{CATALOG}.procurement_silver.silver_projects")
    return df.select(
        col("project_id"),
        col("project_code"),
        col("project_name"),
...
def gold_dim_material():
    df = spark.readStream.table(f"{CATALOG}.procurement_silver.silver_materials")
    return df.select(
        col("material_id"),
        col("material_code"),
        col("material_name"),
...
def gold_dim_contract():
    ...
    return df.select(
        ...
        col("payment_terms"),
        col("retention_pct"),
        col("performance_bond_pct").alias("performance_bond"),
        col("variation_orders").alias("change_order_count"),
        (col("revised_value") - col("contract_value")).alias("change_order_value"),
        when(col("contract_value") > 0,
            spark_round((col("revised_value") - col("contract_value")) / col("contract_value") * 100, 2)
        ).otherwise(lit(0.0)).alias("cost_growth_pct"),
        col("contract_manager_id").alias("approved_by"),
        lit(True).cast(BooleanType()).alias("_is_current"),
        current_timestamp().alias("_gold_timestamp")
    )
...
def gold_dim_sector():
    ...
    return v_sec.union(p_sec).distinct().select(
...
def gold_fact_purchase_orders():
    po = spark.readStream.table(f"{CATALOG}.procurement_silver.silver_purchase_orders")
    poli_agg = (
...
def gold_fact_invoices():
    df = spark.readStream.table(f"{CATALOG}.procurement_silver.silver_invoices")
    return df.select(
        col("invoice_id"),
        col("vendor_id"),
        col("po_id"),
        ...
        col("invoice_type"),
        when(col("payment_date").isNotNull(), lit("PAID"))
            .when(col("due_date") < current_date(), lit("OVERDUE"))
            .otherwise(lit("PENDING")).alias("payment_status"),
        col("invoice_date"),
        col("due_date"),
        col("payment_date"),
        col("invoice_amount"),
...
def gold_fact_goods_receipts():
    gr = spark.readStream.table(f"{CATALOG}.procurement_silver.silver_goods_receipts")
...
def gold_fact_project_costs():
    budgets = spark.readStream.table(f"{CATALOG}.procurement_silver.silver_project_budgets")
    actuals_agg = (
...
def gold_fact_vendor_performance():
    df = spark.readStream.table(f"{CATALOG}.procurement_silver.silver_vendor_performance")
    return df.select(
        col("performance_id"),
        col("vendor_id"),
        col("project_id"),
...
def gold_fact_contracts():
    contracts = spark.readStream.table(f"{CATALOG}.procurement_silver.silver_contracts")
    items_agg = (
    ...
    return df.select(
        ...
        col("payment_terms"),
        col("retention_pct"),
        col("liquidated_damages_pct").alias("penalty_clause"),
        col("performance_bond_pct").alias("performance_bond"),
        col("currency"),
        current_timestamp().alias("_gold_timestamp")
    )
...
def gold_fact_sales():
    sales = spark.readStream.table(f"{CATALOG}.procurement_silver.silver_sales_orders")
