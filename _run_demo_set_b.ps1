...
Write-Host "Executing demo_revert_set_b.sql ..."
$sqlContent = Get-Content -Path 'databricks\scripts\demo_revert_set_b.sql' -Raw -Encoding UTF8
...
foreach ($stmt in $statements) {
    ...
    try {
        ...
        if ($state -eq 'SUCCEEDED') {
            ...
            Write-Host "     $($r.status.error.message)"
            $err++
        }
    } catch {
        $flat    = ($stmt -replace '\s+', ' ').Trim()
...
Write-Host ""
Write-Host "Set B complete: $ok OK, $err ERR"
